...
function(Y,S0=diag(dim(Y)[2]),n0=dim(Y)[2]+2, 
                      nsamp=100,odens=max(1,round(nsamp/1000)),seed=1, 
                      verb=TRUE){
...
Y.imp<-Y
for(j in 1:p) {
Y.imp[is.na(Y[,j]),j]<-quantile(Y[,j],
                       pnorm(Z[is.na(Y[,j]),j],0,sqrt(S[j,j])),na.rm=TRUE,type=1)
                }
Y.pmean<-  ( (ns/odens-1)/(ns/odens) )*Y.pmean+  (1/(ns/odens) )*Y.imp
                  }
...
G.ps<-list(C.psamp=C.psamp,Y.pmean=Y.pmean,LPC=LPC)
class(G.ps)<-"psgc"
return(G.ps)
              }
}
